...
class \nodoc\ iso _TestSetCookieKnownGood is UnitTest
  ...
  fun _test_defaults(h: TestHelper) =>
    match \exhaustive\ SetCookieBuilder("session", "abc123").build()
  ...
  fun _test_all_attributes(h: TestHelper) =>
    match \exhaustive\ SetCookieBuilder("id", "xyz")
      .with_path("/app")
    ...
    | let sc: SetCookie val =>
      ...
      h.assert_true(
        hv.contains("Expires=Thu, 01 Jan 1970 00:00:00 GMT"),
        "all: Expires")
      ...
      try
        ...
        h.assert_true(
          secure_pos < http_only_pos,
          "all: Secure before HttpOnly")
        h.assert_true(
          http_only_pos < same_site_pos,
          "all: HttpOnly before SameSite")
        h.assert_true(
          same_site_pos < max_age_pos,
          "all: SameSite before Max-Age")
        h.assert_true(
          max_age_pos < expires_pos,
  ...
  fun _test_same_site_none(h: TestHelper) =>
    match \exhaustive\ SetCookieBuilder("a", "b")
      .with_same_site(SameSiteNone)
      .build()
    | let sc: SetCookie val =>
      h.assert_true(
        sc.header_value().contains("SameSite=None"),
        "SameSite=None")
    | let e: SetCookieBuildError =>
  ...
  fun _test_same_site_strict(h: TestHelper) =>
    ...
    | let sc: SetCookie val =>
      h.assert_true(
        sc.header_value().contains("SameSite=Strict"),
        "SameSite=Strict")
    | let e: SetCookieBuildError =>
  ...
  fun _test_no_secure(h: TestHelper) =>
    ...
    | let sc: SetCookie val =>
      h.assert_false(
        sc.header_value().contains("Secure"),
  ...
  fun _test_no_http_only(h: TestHelper) =>
    ...
    | let sc: SetCookie val =>
      h.assert_false(
        sc.header_value().contains("HttpOnly"),
  ...
  fun _test_host_prefix(h: TestHelper) =>
    // Valid __Host- usage
    match \exhaustive\ SetCookieBuilder("__Host-session", "abc")
      .with_path("/")
      .build()
    | let sc: SetCookie val =>
      h.assert_true(
        sc.header_value().contains("__Host-session=abc"),
        "__Host-: valid")
    | let e: SetCookieBuildError =>
    ...
    match \exhaustive\ SetCookieBuilder("__Host-session", "abc").build()
    | let sc: SetCookie val =>
  ...
  fun _test_secure_prefix(h: TestHelper) =>
    ...
    match \exhaustive\ SetCookieBuilder("__Secure-token", "xyz").build()
    | let sc: SetCookie val =>
      h.assert_true(
        sc.header_value().contains("__Secure-token=xyz"),
        "__Secure-: valid")
    | let e: SetCookieBuildError =>
    ...
    match \exhaustive\ SetCookieBuilder("__Secure-token", "xyz")
      .with_secure(false)
      .with_same_site(SameSiteLax)
      .build()
    | let sc: SetCookie val =>
  ...
  fun _test_omit_same_site(h: TestHelper) =>
    ...
    | let sc: SetCookie val =>
      h.assert_false(
        sc.header_value().contains("SameSite"),
...
class \nodoc\ iso _TestSetCookieErrors is UnitTest
  ...
  fun apply(h: TestHelper) =>
    ...
    match \exhaustive\ SetCookieBuilder("bad name", "ok").build()
    ...
    match \exhaustive\ SetCookieBuilder("ok", "bad value").build()
    ...
    match \exhaustive\ SetCookieBuilder("ok", "ok")
...
class \nodoc\ iso _PropertySetCookieInvalidNameErrors
  ...
  fun ref property(arg1: String val, ph: PropertyHelper) =>
    match \exhaustive\ SetCookieBuilder(arg1, "ok").build()
    | let _: InvalidCookieName => None // expected
    | let sc: SetCookie val =>
...
class \nodoc\ iso _PropertySetCookieInvalidValueErrors
  ...
  fun ref property(arg1: String val, ph: PropertyHelper) =>
    match \exhaustive\ SetCookieBuilder("validname", arg1).build()
    | let _: InvalidCookieValue => None // expected
    | let sc: SetCookie val =>
