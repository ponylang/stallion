...
- `stallion/` — main package source
  ...
  - `parse_error.pony` — Parse error types (`ParseError` union, 19 error primitives; grammar-aligned per the conformant-by-construction parser. The old overloaded `MalformedHeaders` was retired and removed. `InvalidHostValue` is the protocol-layer Host-value rejection, distinct from the presence/uniqueness `BadHostHeader`)
